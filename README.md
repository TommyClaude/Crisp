...
| `SLACK_WEBHOOK_URL` | no | — | Slack Incoming Webhook URL — when set, posts "new topic + ready draft" messages (both ingestion paths) and "new customer reply + suggested follow-up" messages (mail path) to Slack. Secret: anyone with it can post to the channel |
...
**Fetch-before-create for new topics** — a feed item alone can't tell a genuinely new topic from an old, already-answered one still sitting in a quiet forum's RSS (feed items carry no resolution/reply-role info), so before storing a topic the feed hasn't seen before, the watcher fetches its live page once and applies the same state transitions the email-push path uses: a still-open, customer-last topic is stored, drafted and Slack-notified as before, while a topic that's already resolved on wp.org or where support answered last is stored with the correct flags (so it still surfaces in the right `/suggestions` tab) but is never drafted or announced. If that page fetch fails or parses no posts, the watcher falls back to creating straight from the feed item (the original behavior) so a fetch hiccup never drops a genuinely new topic.
...
Every brand's Crisp website pages its conversation list independently, so "the furthest page reached" is really *one number per brand*, not one global number. `SyncLog.brandPages` (`{[brandId]: {from, to, synced}}`) tracks this per run, updated at the same per-page cadence as the legacy `pageTo` column. When a full/incremental run is interrupted, the dashboard's **Continue** button posts `{mode: "full", resume: true}` — the server derives each configured brand's own furthest page from all of `SyncLog` history (`getResumePages`/`computeResumePages` in `src/lib/sync/sync-service.ts`) and resumes every brand from there automatically, with no page number required. The visible page-number input is kept as a **manual override**, exactly as the owner asked for ("tôi sẽ chủ động điền số trang muốn bắt đầu sync") — it prefills with the selected brand's resume page (`?brand=` in the URL) or, with no brand selected, the first brand's, and is only sent (as `startPage` + `startPageBrandId`, scoping the override to that one brand) when you actually edit it; an untouched prefill is not a "manual" value. `resume` and `startPage` can be combined — the explicit override merges on top of the derived per-brand map, so you can resume everything automatically except for the one brand you want to redirect. `SyncLog.pageFrom` on the resulting row always records the FIRST processed brand's actual starting page (`derivePageFrom` in `sync-service.ts`) — not a stale "1" — so a resumed run's Recent-runs entry doesn't look like it restarted the whole archive. Legacy rows written before `brandPages` existed have no per-brand data and fall back to the old first-brand-only accounting for that one run (see **Full sync** below for the CLI's own legacy-only `--page=N`). The dashboard's **Recent sync runs** Pages column renders `brandPages` as a per-brand range (e.g. "Ninja Team 197→376 · YayCommerce 1→12") when present, falling back to the plain `pageFrom → pageTo` text for legacy rows.

### Crisp's 1,000-page ceiling

Crisp's conversation-list endpoint hard-caps pagination at **page 1000** (undocumented — discovered live on the owner's NinjaTeam brand, which has 20k+ conversations, i.e. 1000+ pages at 20/page): requesting page 1001 always returns `400 {reason: "page_number_too_high"}`, forever, for that website. A full/incremental walk treats that response **exactly like the natural end of that brand's page list** — it stops walking just that brand, records `ceiling: true` on its `SyncLog.brandPages` entry, and the run still finishes `completed` (assuming nothing else failed); other configured brands keep walking normally. The dashboard surfaces it in two places: a short muted line under the sync controls naming the affected brand(s) and pointing at a date-range sync, and a `(ceiling)` suffix on that brand's range in the **Recent sync runs** Pages column. `getResumePages`/`computeResumePages` (`src/lib/sync/sync-service.ts`) read the brand's *latest* run to decide this — Continue (`resume: true`) then **skips that brand entirely** instead of defaulting it to page 1 (a wasted full re-walk) or retrying page 1001 (an immediate 400) — the infinite failure loop the owner hit before this existed. If a later **full or incremental** run for the same brand records a non-ceiling entry (e.g. after Crisp lifts the cap, or a manual page-override walk finds pages again), the flag clears and normal per-brand resume applies again — date-range runs are excluded from resume math entirely (`kind: "range"` rows are always skipped), so range syncs never touch the flag. The reachable-window data (pages 1–1000, i.e. the ~20,000 most recent conversations) is unaffected either way; anything older is reached with **date-range syncs**: pick the brand, then use From/To (or click a month on its heatmap) to walk a window narrow enough to stay under the cap, and repeat backwards until the heatmap shows no gaps.

**For an archive deeper than ~20k conversations per brand**, the page walk simply cannot reach older data through `/conversations/{page}` — use a **date-range sync** instead (the **From/To** fields, or click a month on that brand's coverage heatmap) to pull older windows directly via Crisp's `filter_date_*` params, which aren't subject to the page-1000 limit the same way (a narrow enough window has far fewer than 1000 pages of results).
...
The **Archive coverage** section on `/crisp/dashboard` is a year × month heatmap of the conversations already in the database, bucketed by **last activity** (the `COVERAGE_BASIS` constant in `src/lib/sync/coverage.ts`). A **brand selector** (`?brand=<id>` in the URL) scopes the whole page — stats, coverage, and the range-sync default — to one brand at a time. With one brand selected you get its one grid; with "All brands" you get one **compact grid per brand, stacked vertically**, each with its own total — never a single grid merged across brands, since a filled month for one brand would otherwise hide another brand's gap. Clicking a month prefills the **From / To** date inputs and, in "All brands" mode, remembers which brand's grid you clicked, so that click alone supplies the brand.

**Range syncs are brand-scoped only** — there is no "all brands" range. **Sync range** walks the Crisp list with `filter_date_start`/`filter_date_end` for exactly one brand, named either by the top brand selector or by whichever grid's month you clicked; the button is disabled (with an inline hint) until one of those two supplies a brand, and `POST /api/sync/crisp/start` rejects a date range with no `brandId` with a `400`. Requests are proportional to the window, not the whole archive; every run lands in `SyncLog` as kind `range` with its scoping `brandId` set.
...
- **Backoff** — retryable failures (429/5xx/network) retry with exponential backoff (2s, 4s, 8s, ... capped at 60s), honouring `Retry-After` when present, up to `CRISP_MAX_RETRIES`. Every request/429/backoff wait is counted in a process-wide telemetry singleton (`src/lib/crisp/client.ts`), reset at the start of each run and surfaced on the running-sync card (`GET /api/sync/crisp/status` → `progress.{crispRequests,crisp429s,throttleWaitMs}`) as an amber "Throttled by Crisp" line once backoff is eating a meaningful share of the run's time — visibility a 429-throttled backfill previously had none of. Each 429 also logs `[crisp] 429 — waiting Ns (Retry-After …)` to the server console.
