...
Range syncs are idempotent upserts — re-running a window never duplicates data. Full and incremental syncs are never brand-scoped — they always walk every configured brand — and the "Continue from page" resume number only applies to the *first* brand in that walk; every other brand always starts its own list from page 1 regardless (see the loop comment in `runSync`, `src/lib/sync/sync-service.ts`).
